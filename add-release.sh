...
url=$(./download-package-url.sh --gunzip "https://repo.magento.com/p/magento/magento2-base\$$packageProviderHash.json" | jq ".packages[\"magento/magento2-base\"][\"$RELEASE\"].dist.url" | tr -d '"')
...
url=$(./download-package-url.sh --gunzip "https://repo.magento.com/p/magento/product-community-edition\$$packageProviderHash.json" | jq ".packages[\"magento/product-community-edition\"][\"$RELEASE\"].dist.url" | tr -d '"')
archive="$(basename "$url")"
./download-package-url.sh "$url"
unzip "$archive" composer.json
...
url=$(./download-package-url.sh --gunzip "https://repo.magento.com/p/magento/project-community-edition\$$packageProviderHash.json" | jq ".packages[\"magento/project-community-edition\"][\"$RELEASE\"].dist.url" | tr -d '"')
archive="$(basename "$url")"
./download-package-url.sh "$url"
unzip "$archive" composer.json
